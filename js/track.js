function formatDate(date) {
  ...
  $(document).ready(function($) {
    ...
    $.getJSON("http://test.itdservices.in/api/tracking_api/get_tracking_data?" + searchtype + "=" + tracking_no + "&customer_code=superadmin&company=itd-testing&api_company_id=2", function(data) {
        $('.tracking-data').html('');
        if (data && data.length) {
            $.each(data, function(_index, docket) {
                ...
                if (docket.docket_info.length) {
                    ...
                    var tr_toappend = '<div class="accordion-header  ' + (is_active == 1 ? "active" : "") + '">' +
                      ...
                      '<span class="accordion-toggle"><i class="bx ' + (is_active == 1 ? "icofont-minus" : "icofont-plus") + ' fa_icon"></i></span>' +
                      '</div>' +
                      ...
                      '<td class="w-50">' + (docket.docket_info[0][1]) + '</td>' +
                      '</tr>' +
                      '<tr>'+ 
                      '<th>Booking Date</th>' +
                      ...
                      '<tr>'+ 
                      '<th>Delivery Date</th>' +
                      '<td>' + ((isNaN(new Date(docket.docket_info[5][1])) || (new Date(docket.docket_info[5][1])) == "undefined") ? " " : (formatDate(new Date(docket.docket_info[5][1])))) + '</td>' +
                      '</tr>' +
                      '<tr>'+ 
                      '<th>Delivery Time</th>' +
                      ...
                      '<tr>'+ 
                      '<th>Forwarding No.</th>' +
                      '<td>' + (docket.forwarding_url ? '<a href="' + docket.forwarding_url + '" target="_blank" style="color: black;font-weight: 600;"><u> ' + docket.forwarding_no + '</u></a>' : '' + docket.forwarding_no + '') + '</td>' +
                      '</tr>' +
                      '</tbody>' +
                      '</table>' +
                      '</div>' +
                ...
                if (docket.docket_events.length) {
                    ...
                    tr_toappend += '</tbody>' + '</table>' + '</div>' + '</div>' + '</div>' + '</div>';
        ...
        $('.accordion-header').click(function() {
            if ($(this).hasClass("active")) {
                ...
                $(this).find(".fa_icon").removeClass('icofont-minus').addClass("icofont-plus");
            } else {
                $('.accordion-header').removeClass('active');
                $('.accordion-body').removeClass('active');
                ...
                $(this).find(".fa_icon").addClass("icofont-minus").removeClass('icofont-plus');
            }
        });
    });
  });